...
void* sushi_chef_run(void* arg) {
    ...
    */
    ...
    while (globals_get_sushi_shop_fechado() == 0) {
        ...
        if (next_dish == 0) {
            globals_add_sushi_prod();
        } else if (next_dish == 1) {
            globals_add_dango_prod();
        } else if (next_dish == 2) {
            globals_add_ramen_prod();
        } else if (next_dish == 3) {
            globals_add_onigiri_prod();
        } else {
            globals_add_tofu_prod();
        }

        if (globals_get_sushi_shop_fechado() == 0) {
...
void sushi_chef_seat(sushi_chef_t* self) {  // NÃO PRECISA MODIFICAR, JÁ QUE É APENAS 1 COZINHEIRO
    /* 
        MODIFIQUE ESSA FUNÇÃO PARA GARANTIR O COMPORTAMENTO CORRETO E EFICAZ DO SUSHI CHEF.
        NOTAS:
...
void sushi_chef_leave(sushi_chef_t* self) {
    ...
    */
    ...
    int seat = self->_seat_position;
    conveyor_belt->_seats[seat] = -1;
...
void sushi_chef_place_food(sushi_chef_t* self, enum menu_item dish) {
    ...
    */
    ...
    /* O semáforo a seguir impede que o sushi_chef fique em espera ocupada quando todas as posições de comida estiverem cheias */
    /* Quando o sushi_chef prepara a comida, é feito um "wait" neste semáforo e apenas o cliente faz um "post" quando terminar de comer */

    /* Semáforo que auxilia na espera ocupada do sushi_chef */
    sem_wait(&conveyor_belt->pratos_sem);

    /* Não foi possível impedir a espera ocupada do sushi_chef em todas as situações, apenas quando a esteira está cheia, :( que pena */

    while(conveyor_belt->_food_slots[self->_seat_position] != -1) {

    }

    if (globals_get_sushi_shop_fechado() == 1) {
        return;
    }
